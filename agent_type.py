...
class AgentType:
    ...
    def __init__(self,
                 ...
                 actor_nn               : torch.nn.Module,                      # neural network that maps states -> actions for all agents of this type
                 critic_nn              : torch.nn.Module,                      # neural network used to predict Q value for the actor NN
        ...
        self.actor_policy = copy.deepcopy(actor_nn).to(device)
        self.actor_target = copy.deepcopy(actor_nn).to(device)
        self.actor_opt = optim.Adam(self.actor_policy.parameters(), lr=actor_lr, weight_decay=actor_weight_decay)

        self.critic_policy = copy.deepcopy(critic_nn).to(device)
        self.critic_target = copy.deepcopy(critic_nn).to(device)
